...
def get_messages(
    ...
    msgs = (
        q.order_by(asc(models.Message.created_at))
        .limit(limit)
        .all()
    )
...
async def send_message(
    ...
    # Create delivery statuses for everyone except
    # the sender.
    ...
    # The sender receives the authoritative message
    # through the HTTP POST response, so don't send
    # the same message back through WebSocket.
    recipient_ids = [
        uid
        for uid in participant_ids
        if uid != current_user.id
...
async def mark_read(
    ...
    status_row = (
        db.query(models.MessageStatus)
        .filter(
            models.MessageStatus.message_id
            == message_id,
